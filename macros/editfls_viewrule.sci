function y = editfls_viewrule(filename,var_in,new)
    ieee(2);
    //pause
    fs=loadfls(filename);
...
//pause
    ...
    for cnt = 1:rules_num // cnt is the number of rules
        ...
        for cnt2 = 1:input_num // cnt2 is the number of inputs
            ...
            subplot(rules_num + 2,input_num+output_num,(cnt-1)*(input_num+output_num)+cnt2);//pause
...
//    deff('call_rv(handles)', ['fs = handles.fs;'; 'try';'data = evstr(handles.hedit.string);';...
//    'catch';'data = zeros(1,size(fs.input));';'end';'if  size(data) ~= size(fs.input)';'data = zeros(1,size(fs.input));';...
//    'end';'if  size(data) ~= size(fs.input)';'data = zeros(1,size(fs.input));';'end';'y = flsviewrule(fs,data,1);';'handles = resume(handles);']);
//    
endfunction
