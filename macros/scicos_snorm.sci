...
case 'set' then
	...
	while %t do
	  ...
	  [ok,snorm_type,snorm_par,exprs]=..
      scicos_getvalue('Set snorm block parameters',..
      [//'Channels:';
       'Dubois (0), Yager (1), Drastic Sum (2), Einstein Sum (3), Algebraic Sum (4), Maximum (5):';
	'parameter';],..
	  ...
	  if ok then
	      ...
	      model.ipar=snorm_type;//par(1);
	      model.rpar=snorm_par;//par(2);	
